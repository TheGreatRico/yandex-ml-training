...
class LaplaceDistribution:    
    ...
    def logpdf(self, values):
        ...
        ####
        # Do not change the class outside of this block
        return np.log(1 / (2 * self.scale)) - np.abs(values - self.loc) / self.scale # np.log(self.pdf(values)) # 
        ####
    ...
    def pdf(self, values):
        ...
        Args:
            ...
            Every column represents all available values for the selected feature.
        '''
        return np.exp(self.logpdf(values)) # np.exp(- np.abs(values - self.loc) / self.scale) / (2 * self.scale) # 
